...
#pragma config(Motor,  motorA,          Port,        tmotorNXT, PIDControl, encoder)
#pragma config(Motor,  motorB,          Starboard,         tmotorNXT, PIDControl, encoder)
...
#define COMMAND_TURN_LASER_CENTRE 	8
...
{
	static char acWriteBuffer[5];
	static char acReadBuffer[4];

	memset(acReadBuffer, 0, sizeof(acReadBuffer));

	acWriteBuffer[0] = 5;    						// Messsage Size
	acWriteBuffer[1] = ARDUINO_ADDRESS;
	acWriteBuffer[2] = ELF_MSG_HEADER;	// our header
	acWriteBuffer[3] = command;				 	// Command
	acWriteBuffer[4] = (0-command)%256;	// checksum

	sendI2CMsg(ARDUINO_PORT, acWriteBuffer, 4);
	delay(20);
...
//
//landlubber equivalent: turn the robot around to its left
//

void turnPort()
{
	motor[Port] = 100;				//turn port motor to full forward power
	motor[Starboard] = -100;	//turn starboard motor to full backward power
}


//
// turn the robot around to its right
//

void turnStarboard()
{
	motor[Port] = -100;				//turn port motor to full backward power
	motor[Starboard] = 100;		//turn starboard motor to full forward power
}
...
void goBow()
{
	motor[Port] = 100;				//see previous methods
	motor[Starboard] = 100;
}
